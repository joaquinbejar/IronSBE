...
impl SmoltcpStackInner {
    ...
    fn add_listener(&mut self) -> SocketHandle {
        ...
        // Best-effort: any errors here mean the listen is mis-configured;
        // surface as a panic-free debug log and skip.  In a production
        // path the caller would surface this through the stack error.
        let _ = sock.listen(self.config.local_port);
    ...
    fn poll(&mut self) {
        ...
        for (idx, session) in self.sessions.iter_mut().enumerate() {
            ...
            // Push outbound frames if the socket can send.
            while let Some(msg) = session.tx_queue.front() {
                if !sock.can_send() {
                    break;
                }
                let frame_len_u32 = match u32::try_from(msg.len()) {
                    Ok(v) => v,
                    Err(_) => break, // dropped silently; surface in connection.send
                };
...
impl SmoltcpStack {
    ...
    ///
    /// # Panics
    /// Panics if smoltcp rejects the IP/MAC configuration (only happens
    /// for malformed input that the caller could have caught at
    /// construction time).
    #[must_use]
    pub fn new(config: SmoltcpStackConfig) -> Self {
    ...
    /// Returns the next newly accepted session, if any.
    #[must_use]
    pub fn accept(&self) -> Option<SmoltcpConnection> {
        let mut inner = self.inner.borrow_mut();
        let session_idx = inner.pending_accepts.pop_front()?;
        Some(SmoltcpConnection {
            session_idx,
...
pub enum SmoltcpStackError {
    /// A frame could not be processed by the smoltcp interface.
    #[error("smoltcp poll error: {0}")]
    Poll(String),
}
...
impl XdpStack for SmoltcpStack {
    ...
    fn on_rx(
        ...
        // Push the inbound frame into the device rx queue.
        ...
        // Drain any frames smoltcp wants to send.
        ...
        // Hand back any newly-accepted session.
        Ok(self.accept())
    }
...
mod tests {
    ...
    #[test]
    fn test_no_pending_accept_initially() {
        let cfg = SmoltcpStackConfig::new(LOCAL_IP, 24, 9000, LOCAL_MAC);
        let stack = SmoltcpStack::new(cfg);
        assert!(stack.accept().is_none());
    }
}
